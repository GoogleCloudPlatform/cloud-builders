...
NAMESPACE="test-gcb-run"
OUTPUT="/var/tmp/gke-deploy-test/test_gcb_run"
...
# Execute and Verify
gcloud builds submit --config cloudbuild_gcb_run.yaml . --project="${GKE_DEPLOY_PROJECT}" --substitutions="_CLUSTER=${GKE_DEPLOY_CLUSTER},_LOCATION=${GKE_DEPLOY_LOCATION},_NAMESPACE=${NAMESPACE}" \
